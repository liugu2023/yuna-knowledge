...
这篇文档按实际使用过程解释这些词。资料核对日期为 2026 年 7 月 10 日；模型能力、产品名称和协议版本仍可能变化。
...
不用一次记住所有缩写。按下面的顺序理解就够用：

1. 先学会写清楚 Prompt，知道模型会产生幻觉，重要信息要查原文。
2. 理解 Token 和 Context window。上传文件前，知道模型不一定能完整读完所有材料。
3. 经常用课件、论文或规章问答时，再理解 RAG、Embedding 和 Grounding。
4. 使用 AI 编程或自动化工具时，重点看 Tool calling、Agent、权限确认和 MCP。
5. 想在本地运行或训练模型，再研究 Quantization、Fine-tuning 和 LoRA。

还要守住几条很实际的安全边界：

- 不要把密码、API Key、身份证号或未公开的个人材料直接发给不清楚数据政策的服务。
- 涉及个人信息、未公开论文和实验数据时，检查服务商的数据存放地区、日志保留和跨境处理方式。
- 课程作业能否使用生成式 AI，以任课教师和学校要求为准。引用、数据和实验结果必须自己核对。
- 给 Agent 开文件、命令行、邮箱或网盘权限时，从最小范围开始，不要图省事直接开放全部目录。
- 模型说“已经完成”不代表真的完成。检查文件、日历、邮件和命令执行结果。
