...
**Workers** 可以在边缘节点运行 JavaScript/TypeScript（也支持 Python、Rust 编译到 WASM），适合写小型 API、Webhook、定时任务、反向代理等。搭配 KV / D1 / R2 就是一套完整的免费全栈方案。
...
**Cloudflare Tunnel**（`cloudflared`）可以把本地或校园网内的服务暴露到公网，不需要公网 IP、不需要开端口，配合自己的域名使用。对于在宿舍跑 NAS / 自建服务的同学非常实用。
