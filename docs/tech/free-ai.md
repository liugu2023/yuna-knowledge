...
> 学习大模型开发、给自己的项目接入 AI 能力，不一定要花钱买 API。本文汇总 **2026 年**主要平台的免费额度。
>
> ⚠️ 免费额度是所有云服务里变动最快的（曾有平台一夜之间砍掉 80% 以上的免费额度），使用前务必去官网核实。
...
- ⚠️ 两个坑：免费层的数据会被 Google 用于改进产品（勿传敏感内容）；2025 年 12 月 Google 曾无预告大砍免费额度（Flash 日请求数从约 250 次降到 20~50 次），额度随时可能再变
...
- 自研 LPU 芯片，推理速度极快（首字延迟通常 200ms 以内），适合实时对话类应用
...
- **GLM-4-Flash、GLM-Z1-Flash 永久免费**、不限 token 量（只限并发数），是国内免费接口里最稳的兜底选择
...
1. **多平台组合**：各平台的速率限制相互独立，Gemini + Groq + OpenRouter + 智谱组合使用，免费容量成倍增加
2. **写好重试逻辑**：免费 API 都有 RPM / RPD 限制，代码里加指数退避重试，429 错误自动等待
3. **做好 fallback**：一个平台挂了自动切下一个（可配合 [cc-switch](./cc-switch.md) 或网关类工具管理多个供应商）
4. **免费层只用于学习和开发**：生产环境需要付费版的 SLA 保障
5. 需要经国内中转访问海外模型的场景，请先阅读 [API 中转站简介](./relay.md)了解风险

<!-- TODO: 待补充各平台在校园网环境下的实测连通性；各家具体限额数字变动极快，此处只保留量级参考，精确数值以官网为准 -->
